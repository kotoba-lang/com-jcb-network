(ns jcb_network.main-test
  (:require [clojure.test :refer [deftest is testing run-tests]]
            [clojure.string :as str]
            [jcb_network.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "jcb_network-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'jcb_network.main-test)]
