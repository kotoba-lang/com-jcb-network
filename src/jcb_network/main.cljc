(ns jcb_network.main
  "Kotodama WASM entrypoint for the Jcb_network clean-room actor (L4) — Clojure port.
  ...
  facts (`jcb_network.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jcb_network")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"jcb_network.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jcb_network-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jcb_network-compat/ready)
